...
<html>
	...
	<style>
		...
		body{
				background-image: url("images/newton1.jpg");
	...
	<head>
		<h1 align="middle" style="font-size: 500%;"><u>Newton</u></h1>
	</head>
	...
	<body  style="background-color: powderblue; font-family: monospace; margin: 0; padding: 0;">
		...
		<p align=middle>
			<video width="700" height="545" controls >
				<source src="video/newton.mp4" type="video/mp4">
			</video>
		...
		<?php
			...
			$sql="SELECT description,genre,mov_lang,dir_name,stars from movie m,director d,rating r where d.dir_id=m.dir_id and r.mov_id=m.mov_id and mov_name='Newton'; ";
		...
		<?php
			$sql1="SELECT actor_name,role from actor a,movie m,movie_cast mc where a.actor_id=mc.actor_id and mc.mov_id=m.mov_id and m.mov_name='Newton';";
			mysqli_query($conn, $sql) or die('Error');
